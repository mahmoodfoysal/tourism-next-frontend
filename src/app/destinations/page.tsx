...
const DestinationsPage = () => {
  ...
  useEffect(() => {
    ...
    if (selectedBadge !== "All") {
      filtered = filtered.filter((dest) => dest.badge === selectedBadge);
    }
  ...
  }, [searchQuery, priceRange, selectedBadge, sortBy, allDestinations]);

  const badges = ["All", ...new Set(allDestinations.map((d) => d.badge))];
            ...
            >
              <div className="sticky top-28 space-y-6">
                    ...
                    {badges.map((badge) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 ${
                          selectedBadge === badge
                            ? "bg-primary text-white shadow-lg shadow-primary/20 -translate-y-0.5"
                ...
                <button
                  ...
                  className="w-full py-4 rounded-2xl border border-dashed border-error/20 text-error/60 text-xs font-black uppercase tracking-widest hover:bg-error hover:text-white hover:border-transparent transition-all duration-300 group"
                >
