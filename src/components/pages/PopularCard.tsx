...
const PopularCard: React.FC<PopularCardProps> = ({ info }) => {
    return (
        <div key={info.pop_id} className="group flex flex-col bg-base-100 rounded-[2.5rem] overflow-hidden border border-base-content/5 shadow-xl hover:shadow-2xl transition-all duration-500 h-full">
            {/* Image Section */}
            <div className="relative h-72 overflow-hidden">
                ...
                <div className="absolute top-5 right-5">
                    <div className="bg-base-100/90 backdrop-blur-md px-4 py-2 rounded-2xl shadow-xl flex flex-col items-end border border-white/10">
                        <span className="text-[10px] font-black uppercase text-base-content/30 tracking-tighter leading-none">From</span>
                        <span className="text-lg font-black text-primary leading-none mt-1">${info.price}</span>
                    </div>
                </div>
                ...
                <div className="pt-6 border-t border-base-content/5 mt-auto flex items-center justify-between">
                    <div className="flex -space-x-2">
                        {info.moreImage.slice(0, 3).map((img, i) => (
                            <div key={i} className="w-8 h-8 rounded-full border-2 border-base-100 overflow-hidden relative shadow-sm">
                    ...
                    <button className="flex items-center gap-2 text-primary text-[11px] font-black uppercase tracking-widest group-hover:gap-3 transition-all">
                        Explore Now
