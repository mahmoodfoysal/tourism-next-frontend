 ...
 */
const authenticateAndSync = async (email: string, fullName: string, shouldSync: boolean = false) => {
    try {
        // 1. Fetch JWT Token
        ...
        if (token) {
            ...
            // 2. Sync with backend ONLY if it's a new user
            if (shouldSync) {
...
export const registerUser = createAsyncThunk(
    ...
    async ({ email, password, fullName }: { email: string; password: string; fullName: string }, { rejectWithValue }) => {
        try {
            ...
            await authenticateAndSync(email, fullName, true);
...
export const loginWithGoogle = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            await authenticateAndSync(user.email || '', user.displayName || 'Google User', isNewUser);
